...
$( document ).ready(function() {
	...
	$('.desc-avail-table').each(	function( index ) { // load availability
		...
		{ myAjaxUrl = 'https://webrooms.dev64.welman.co.nz' } //dev
	...
	$('.webrooms-datepicker-pagechooser a').each(	function( index ) { // init date pickers
		...
		var oPagechooser = $( '.webrooms-datepicker-pagechooser select' ).get(0);
		
		var currentPathName = window.location.pathname.split('/');
		currentPathName = currentPathName[currentPathName.length-1];
		
		if($(oPagechooser).find('option[value="'+currentPathName +'"]').length > 0)
		{
			$(oPagechooser).val(currentPathName);
		}
		...
		$( oButton ).click(function() {	
			if($(oPagechooser).val() == '')
			{	$(oPagechooser).parent().effect('highlight', {'duration':5000})	}
			else
			{	window.location.href=$(oPagechooser).val()+'?time='+$(oPicker).datepicker('getDate').getTime(); }
		});
