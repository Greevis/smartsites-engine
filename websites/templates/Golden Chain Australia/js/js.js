...
$( document ).ready(function() {
	...
	$('.desc-avail-table').each(	function( index ) { // load availability
		...
		var myNights = parseFloat(getQuerystring('nights'));
		if (isNaN(myTime))
		{
			myTime = new Date();
		...
		if(myNights>0)
		{
			sQuery += '&nights='+myNights;
		}
		var sGridQuery = sQuery; 
		sQuery += '&time='+myTime;
		var myAjaxUrl;
		if(window.location.host.indexOf('welman.co.nz') > 0)
		{ myAjaxUrl = 'https://webrooms.bishopx.welman.co.nz' } //dev
		...
		$(this).load(myAjaxUrl, function(responseTxt,statusTxt,xhr) {
			...
			$(sSel).click( function() {
				...
				var dayOffSet = $(sSel).index($(this));
			
				var arrIds = $(this).parents('.desc-avail-table').prop('id').split('-');
				
				var thisCellTime = myTime + (dayOffSet * (1000*60*60*24))
				
				var thisGridQuery = sGridQuery + '&time='+thisCellTime;		 
				if(arrIds.length > 2)
				{
					thisGridQuery += '&rooms='+arrIds[3];
				}
			 	thisGridQuery += '#book';					
			 	
			 	var sUrl = $(this).parents('.desc-avail-table').siblings('a.property-view').first().prop('href');
			 	if(sUrl)
			 	{ // property summary link
			 		sUrl = sUrl.split('?')[0]+thisGridQuery; 
			 	}else{ // roomtype summary link
			 		sUrl = $(this).parents('.desc-avail-table').siblings('a.book-roomtype').first().prop('href');
			 		sUrl = sUrl.split('?')[0]+thisGridQuery; 
				}
	...
	$('a.property-view, ul.nav li a, .property-name a').each(	function( index ) { // maintain time stamp on these links		
		...
		var thisNights = getQuerystring('nights',0);
		if(isNaN(thisNights) || thisNights == 1){
			thisNights = 1;
		}
		...
		{thisPrefix = '&' }
		if(thisTimestamp > 0)
		{	$(this).prop('href',thisUrl+thisPrefix+'time='+thisTimestamp+'&nights='+thisNights);}		
	});
	...
	$('.desc-book-button, .webrooms-datepicker a, .book-roomtype, .book-link').each(	function( index ) { // maintain time stamp on these links and add #book hash
		...
		if(thisTimestamp > 0)
		{	
			$(this).prop('href',thisUrl+thisPrefix+'time='+thisTimestamp+'&nights='+thisNights+'#book');
		}else{
	...
	$('.webrooms-datepicker-custom a').each(	function( index ) { // init date pickers
		...
		oButton = $('<button type="button" class="datepicker-button ix-'+index+'">'+sText+'</button>');	
